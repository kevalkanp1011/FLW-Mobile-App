...
interface SyncDao {
    ...
    @Query(
        "       SELECT 1 as id, 'Beneficiary' as name, b1.syncState as syncState, count(*) as count from beneficiary b1 group by b1.syncState "
                ...
                + "UNION SELECT 13 as id, 'Delivery Outcome' as name,  do.syncState as syncState, count(*) as count from delivery_outcome do group by do.syncState "
                ...
                + "UNION SELECT 15 as id, 'PMSMA' as name,  pmsma.syncState as syncState, count(*) as count from PMSMA pmsma group by pmsma.syncState "
                + "UNION SELECT 16 as id, 'Immunization' as name,  imm.syncState as syncState, count(*) as count from IMMUNIZATION imm group by imm.syncState "
                 + " ORDER by id")
