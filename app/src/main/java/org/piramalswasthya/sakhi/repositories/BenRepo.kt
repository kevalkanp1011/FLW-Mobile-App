...
//        }
    ...
    suspend fun syncUnprocessedRecords(): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            val benUnprocessedList = database.benDao.getAllUnsyncedBen()
            ...
            benUnprocessedList.forEach {
                ...
                val hh = database.householdDao.getHousehold(it.householdId)!!.asNetworkModel(user)
...
//                        Timber.d("Yuuhooo  -- ---${benNetworkPostSet.first().benId}  ${householdNetworkPostSet.first().householdId}")
                        ...
                        hhToUpdateList?.let { database.householdDao.householdSyncedWithServer(*it) }
                        cbacToUpdateList?.let { database.cbacDao.cbacSyncedWithServer(*it) }
