...
@HiltWorker
class PullInfantRegFromAmritWorker @AssistedInject constructor(
    ...
    companion object {
        const val name = "PullInfantRegFromAmritWorker"
        ...
        return try {
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    val result1 =
                        awaitAll(
                            async { getInfantRegDetails() }
                        )

                    val endTime = System.currentTimeMillis()
                    val timeTaken = TimeUnit.MILLISECONDS.toSeconds(endTime - startTime)
                    Timber.d("Full infant reg details fetching took $timeTaken seconds $result1")
